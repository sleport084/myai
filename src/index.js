// 网络代理必须在所有 fetch/LLM 调用之前配置好全局 dispatcher，故放在文件最顶部。
import './network-proxy.js'
import { config, getMinimaxKey as _getMinimaxKey, getSecurity, getMediaCredentials } from './config.js'
...
import { registerProvider, replaceProvider } from './providers/registry.js'
...
import { formatSelfEvolutionForPrompt, recordSelfEvolutionFromMemories } from './memory/self-evolution.js'
import { startTyphoonAlertMonitor } from './typhoon-alert-monitor.js'
import { startRelayPoller } from './relay-poll.js'
import { startMcpClients, shutdownMcpClients } from './mcp/client-manager.js'
...
// 注册 OpenAI 图片生成 provider（如果配置了 key）
function registerOpenAIImageIfAvailable() {
  const creds = getMediaCredentials()
  if (creds.openaiImageKey) {
    registerProvider(new OpenAIImageProvider({
      apiKey: creds.openaiImageKey,
      baseURL: creds.openaiImageBaseURL || 'https://api.openai.com/v1',
      model: creds.openaiImageModel || 'dall-e-3',
    }))
    console.log('[Provider] OpenAI Image (DALL-E) registered')
  }
  if (creds.customImageKey && creds.customImageUrl) {
    registerProvider(new OpenAIImageProvider({
      name: 'custom-image',
      apiKey: creds.customImageKey,
      baseURL: creds.customImageUrl,
      model: creds.customImageModel || 'dall-e-3',
    }))
    console.log('[Provider] Custom Image registered')
  }
}
registerOpenAIImageIfAvailable()
...
configureRecognizerScheduler({
  onResult: (memories) => {
    ...
    if (Array.isArray(memories) && memories.length > 0) {
      refreshUserProfile(PRIMARY_USER_ID)
      // 自我进化：从约束/策略/教训类记忆中提炼可执行规则，7 天内不重复提示
      try {
        const evolved = recordSelfEvolutionFromMemories(memories, { emitEvent })
        if (evolved?.length) console.log(`[self-evolution] learned ${evolved.length} behavior update(s)`)
      } catch (err) {
        console.warn('[self-evolution] record failed:', err?.message || err)
      }
    }
  },
})
...
function buildSystemEnv(msg) {
  ...
  // 飞书连接状态：仅在用户提到飞书时注入（避免无谓占用上下文）
  if (/飞书|feishu|lark/i.test(text)) {
    const feishuBlock = getFeishuStatusBlock()
    if (feishuBlock) blocks.push(feishuBlock)
  }
  // 终端流预览窗口状态：会话级信号，有活跃窗口时常驻（内部无窗口返回空串）
  const terminalStreamBlock = formatTerminalStreamContext()
  if (terminalStreamBlock) blocks.push(terminalStreamBlock)
  // 自我进化记忆：Agent 从约束/教训中提炼的可执行规则
  const selfEvoBlock = formatSelfEvolutionForPrompt()
  if (selfEvoBlock) blocks.push(selfEvoBlock)
...
async function runTurn(input, label, msg = null) {
  ...
  try {
    ...
    const agentName = getConfig('agent_name') || 'MyAI'
...
async function startConsciousnessLoop({ runImmediateTick = true } = {}) {
  ...
  // 台风预警自动监测：5 分钟轮询官方预警流，仅在正式橙/红预警时触发大屏。
