...
// 调用某能力（自动路由，可指定 provider 名字）
export async function callCapability(capability, params, providerName) {
  let provider
  if (providerName) {
    provider = getProviderByName(providerName)
    if (!provider) throw new Error(`没有找到名为 "${providerName}" 的 Provider`)
    if (!provider.canDo(capability)) throw new Error(`Provider "${providerName}" 不支持能力 "${capability}"`)
  } else {
    provider = getProvider(capability)
  }
