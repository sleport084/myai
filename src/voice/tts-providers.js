...
export const TTS_PROVIDERS = [
  ...
  { id: 'custom',      label: '自定义（OpenAI 兼容）', streaming: true },
]
...
export const TTS_PROVIDER_REQUIREMENTS = {
  ...
  custom: {
    label: '自定义 TTS',
    groups: [
      { keys: ['customTtsUrl'], label: 'Base URL' },
      { keys: ['customTtsKey'], label: 'API Key' },
    ],
    guide: '请在「语音设置 → 语音合成」里选择自定义，并填写 Base URL（如 https://your-api.com/v1/audio/speech）和 API Key。',
  },
}
...
// ── 通用入口 ────────────────────────────────────────────────────────────────
// ── 自定义 TTS（OpenAI 兼容格式）──────────────────────────────────────────
// 支持任何兼容 OpenAI /v1/audio/speech 接口的 TTS 服务
async function streamCustom({ text, voiceId = 'alloy', apiKey, baseURL, model }) {
  ...
  switch (provider) {
    ...
    case 'custom':
      return streamCustom({
        text,
        voiceId,
        apiKey: keys.customTtsKey,
        baseURL: keys.customTtsUrl,
        model: keys.customTtsModel,
      })
    default:
