...
// ─── 自定义 ASR（OpenAI Whisper 兼容）───
// 非流式：缓冲音频，flush 时发送 HTTP POST 到 /v1/audio/transcriptions
function createCustomASRSession(config, lang, onTranscript, onError, onClose) {
  const baseUrl = (config.customAsrUrl || '').replace(/\/+$/, '')
  const apiKey = config.customAsrKey || ''
  const model = config.customAsrModel || 'whisper-1'
  if (!baseUrl) { onError('自定义 ASR: 缺少 Base URL'); return null }
  if (!apiKey) { onError('自定义 ASR: 缺少 API Key'); return null }
  ...
  return {
    ...
    async flush() {
      if (closed || !chunks.length) return
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          onError(`自定义 ASR 失败 (${resp.status}): ${err.slice(0, 200)}`)
          return
        }
        const data = await resp.json()
        const text = data.text || data?.result?.text || ''
        if (text) onTranscript(text, true, null)
      } catch (err) {
        onError(`自定义 ASR 请求失败: ${err.message}`)
      }
    },
...
export function createCloudASRSession(config, onTranscript, onError, onClose, onEvent) {
  ...
  if (provider === 'custom') {
    return createCustomASRSession(config, lang, onTranscript, onError, onClose)
  }

  // macOS 原生离线 ASR（Speech.framework）：完全离线、无需 API Key。
  // 仅 macOS 可用；预编译 universal binary（x86_64+arm64）在 build/native-speech-recognizer。
  if (provider === 'local') {
    if (process.platform !== 'darwin') {
      onError('本地 ASR (macOS Speech) 仅支持 macOS')
      return null
    }
    return createMacSpeechSession({ lang, mode: config.macosRecognitionMode || 'auto' }, onTranscript, onError, onClose, onEvent)
  }
