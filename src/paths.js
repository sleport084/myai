...
export const paths = {
  ...
  // 用户自定义 MCP server 配置（stdio 命令、环境变量引用），0600 权限落盘。
  mcpServersFile: path.join(USER_DIR, 'mcp-servers.json'),
  // API 能力槽位配置（如 Kimi vision）：记录每个槽位用哪个 provider/模型。
  apiCapabilitySlotsFile: path.join(USER_DIR, 'api-capability-slots.json'),
  // 第三方凭证密文（API key 等）：safe-storage 不可用时回退到 AES-256-GCM 主密钥加密。
...
export function rescueDataFromInstallDir() {
  ...
  let installDir
  try {
    // exe 所在目录就是安装目录（process.execPath = <install>\MyAI.exe）
  ...
  // dedicated MyAI install folder.
  if (path.basename(installDir).toLowerCase() !== 'xiaobailong') {
