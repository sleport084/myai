...
import { getDB, getConfig, setConfig, insertUISignal, getUnconsumedUISignals, upsertMediaHistory, getMediaHistory, updateLastJarvisConversationContent, getRecentRecallAudits, getRecentExtractAudits, getRecallAuditStats, getExtractAuditStats } from './db.js'
...
import { config, activate as activateLLM, prepareActivation as prepareLLMActivation, commitPreparedActivation, getActivationStatus, switchModel, saveLLMSettings, setTemperature, setThinking, getMinimaxKey, setMinimaxKey, getSocialConfig, setSocialConfig, getVoiceConfig, setVoiceConfig, getTTSConfig, setTTSConfig, getTTSCredentials, getProviderSummaries, getSecurity, setSecurity, getEmbeddingConfig, setEmbeddingConfig, EMBEDDING_PROVIDER_PRESETS, getWebSearchConfig, setWebSearchConfig, getMediaConfig, setMediaConfig, getMediaCredentials, getContextWindowConfig, setContextWindowConfig, getHeartbeatConfig, setHeartbeatConfig } from './config.js'
...
import { handleSceneConnection } from './scene/scene-server.js'
...
import { reconcileMcpClients, getMcpStatus } from './mcp/client-manager.js'
import { getMapRuntimeConfig, getMapServiceSettings, setMapServiceSettings } from './map-service.js'
...
export function startAPI(port = 3721, { getStateSnapshot = null, onActivated = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // GET /events/history — recent UI signal history (fork 无持久化 SSE replay,用 ui_signals 近似)
    if (req.method === 'GET' && url.pathname === '/events/history') {
      const limit = Math.min(Math.max(Number(url.searchParams.get('limit') || 160), 1), 1000)
      const windowMs = Math.max(limit, 160) * 60000
      const signals = getUnconsumedUISignals(windowMs)
      const events = signals.slice(-limit).map(row => ({
        id: row.id,
        type: row.type,
        target: row.target,
      ...
      jsonResponse(res, 200, {
        ok: true,
        events,
        latest_event_id: events.length ? events[events.length - 1].id : 0,
        has_more: false,
      })
      return
    }
    ...
    // GET /typhoons — active typhoon list (中央气象台台风网,10分钟缓存)
        ...
        readJsonBody(req)
          .then((body) => {
            const active = typeof body.active === 'boolean'
              ? body.active
              : /^(1|true|yes|open|show)$/i.test(String(body.active || ''))
            const state = setTyphoonPanelState({ active, source: body.source || 'brain-ui' })
            jsonResponse(res, 200, { ok: true, state })
          })
          .catch((err) => jsonResponse(res, 400, { ok: false, error: err.message }))
        return
    ...
    // GET /social/feishu/status — 飞书连接状态(未装 SDK 时返回 disabled)
    if (req.method === 'GET' && url.pathname === '/social/feishu/status') {
      jsonResponse(res, 200, getFeishuStatus())
      return
    }

    // GET /map-service/config — 高德地图运行时配置(前端 MapService 加载时拉取)
    if (req.method === 'GET' && url.pathname === '/map-service/config') {
      jsonResponse(res, 200, getMapRuntimeConfig())
      return
    }

    // GET/POST /settings/map — 高德地图 JS Key + 安全密钥配置
    ...
    // GET/POST /settings/mcp — MCP server 配置(用户自定义 stdio MCP server)
    if (req.method === 'GET' && url.pathname === '/settings/mcp') {
      jsonResponse(res, 200, { ok: true, mcp: getMcpServersConfig(), status: getMcpStatus() })
      return
    }
        ...
        .then(async (body) => {
          try {
            ...
            emitEvent('mcp_settings_updated', {
              ...
              enabledCount: mcp.servers.filter(s => s.enabled).length,
    ...
    // POST /settings/context-window — save context window configuration
    ...
    // GET /settings/agent-name — read the agent display name
    if (req.method === 'GET' && url.pathname === '/settings/agent-name') {
      jsonResponse(res, 200, { ok: true, agent_name: getAgentName() })
      return
    }

    // POST /settings/agent-name — save the agent display name
    ...
    // scene-shell 声明式 UI 静态资源（与 brain-ui 同模式，路径隔离）
    if (req.method === 'GET' && url.pathname.startsWith('/src/ui/scene-shell/')) {
      const sceneShellRoot = path.resolve(paths.resourcesDir, 'src', 'ui', 'scene-shell')
      ...
      if (!isPathInside(sceneShellRoot, assetPath)) { res.writeHead(403); res.end('forbidden'); return }
      try {
        const stat = fs.statSync(assetPath)
        if (!stat.isFile()) { res.writeHead(404); res.end('asset not found'); return }
        res.writeHead(200, {
          'Content-Type': contentTypeFor(assetPath),
          'Content-Length': stat.size,
          'Cache-Control': 'no-cache',
    ...
    // GET /terminal-stream — 终端流预览窗口页面（独立 sidecar 窗口加载）
    if (req.method === 'GET' && url.pathname === '/terminal-stream') {
      const htmlPath = path.resolve(paths.resourcesDir, 'src', 'ui', 'terminal-stream', 'index.html')
      try {
        ...
        res.writeHead(404)
        res.end('terminal-stream page not found')
      }
      return
    }

    // GET /terminal-stream/history — 终端流历史片段（前端启动时拉取，之后走 /events SSE）
    ...
    // GET /settings/media — read media generation configuration
    if (req.method === 'GET' && url.pathname === '/settings/media') {
      jsonResponse(res, 200, { ok: true, media: getMediaConfig() })
      return
    }

    // POST /settings/media — save media generation configuration
    if (req.method === 'POST' && url.pathname === '/settings/media') {
      const chunks = []
      req.on('data', chunk => chunks.push(chunk))
      req.on('end', () => {
        try {
  ...
  // scene 声明式 UI 通道：与 /acui 命令式通道并存。
  // ui_set 工具写入 sceneStore，sceneStore 通过这里的 WS 把变更广播给 scene-shell 前端。
  ...
  // 端口被占时不让进程裸崩（Unhandled 'error' event），给出清晰提示并退出
  server.on('error', (err) => {
    if (err?.code === 'EADDRINUSE') {
      console.error(`[API] 端口 ${port} 已被占用，可能有一个 MyAI 实例正在运行。请先退出其他实例再重试。`)
      console.error(`[API] 原始错误: ${err.message}`)
    } else {
      console.error(`[API] HTTP server 错误: ${err?.message || err}`)
    }
    process.exit(1)
  })
