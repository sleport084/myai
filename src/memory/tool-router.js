...
// ---- 工具分组 ----
//
// core：任何场景都注入。ACUI 工具默认带上（MyAI侧 Phase 1 决策，组件少 token 便宜）。
const CORE_TOOLS = [
  'send_message',
  'recall_memory',
...
// 能力演示：用户问"你能做什么"时注入（本地可视渠道专属）
const CAPABILITY_DEMO_TOOLS = ['capability_demo']
// 软件安装（Windows winget 专属，macOS 调用会返回拒绝但 schema 仍可暴露）
...
export function selectTools(ctx = {}) {
  ...
  // 能力演示：本地可视渠道 + 用户问"你能做什么"时注入
  if (!isTick && shouldInjectCapabilityDemo(messageBody)) {
    for (const t of CAPABILITY_DEMO_TOOLS) out.add(t)
  }
  // 软件安装：用户明确要装软件时注入（winget Windows 专属，macOS 会返回拒绝提示）
  if (!isTick && isSoftwareInstallRequest(messageBody)) {
  ...
  // 保活只覆盖MyAI的"已知工具"——installed 工具走单独的全注入路径。
