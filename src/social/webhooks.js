...
// 解析飞书事件回调里的消息内容（供 feishu-ws 长连接和 webhook 两种接入方式共用）
...
// /social/feishu/status 等是状态查询路由,不是 webhook 回调,要排除掉避免被 handleSocialWebhook 拦截
const SOCIAL_NON_WEBHOOK_PATHS = new Set(['/social/feishu/status'])

export function isSocialWebhookPath(pathname) {
  if (!pathname || !pathname.startsWith('/social/')) return false
  return !SOCIAL_NON_WEBHOOK_PATHS.has(pathname)
}
...
async function handleFeishu(req, res) {
  ...
  if (body.encrypt) return jsonResponse(res, 400, { ok: false, error: 'encrypted Feishu events are not enabled in MyAI yet' })
