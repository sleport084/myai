...
import { execWechatSnapshot, execWechatClick, execWechatType, execWechatKey, execWechatFind } from './tools/wechat.js'
...
import { isMcpTool, executeMcpTool } from '../mcp/client-manager.js'
  ...
  try {
    ...
    switch (name) {
      ...
      case 'wechat_snapshot':
        return await execWechatSnapshot(args)
      case 'wechat_click':
        return await execWechatClick(args)
      case 'wechat_type':
        return await execWechatType(args)
      case 'wechat_key':
        return await execWechatKey(args)
      case 'wechat_find':
        return await execWechatFind()
      ...
      default:
        // MCP 工具：别名形如 mcp__<server>__<tool>,由 client-manager 路由到对应 MCP server
        if (isMcpTool(name)) {
...
// 能力演示：用户问"你能做什么"时启动可视化演示（天气卡→流式写文章→热点面板）。
// 外部渠道（微信/Discord/飞书）不投屏，直接拒绝让模型用文字回答。
