...
const ROLE_RULES = [
  {
    ...
    keywords: [/xiaobailong/i, /agent/i, /llm/i, /prompt/i, /context/i, /memory/i, /codex/i, /claude/i, /openai/i, /minimax/i],
...
function inferProjects(text) {
  const projects = []
  if (/xiaobailong/i.test(text) || /MyAI/.test(text)) projects.push('MyAI')
